# Coverage

Generated 2026-09-10 · [live JSON](data/v1/coverage.json)

Every company in the registry, with what we actually pulled from its board.
A zero in **Published** is not automatically a gap — it may mean the company
has no early-career roles open right now, which is itself worth knowing.

| | |
|:--|--:|
| Companies in registry | 145 |
| Postings fetched | 21,211 |
| Postings published | 335 |
| Boards that failed to fetch | **1** |
| Boards live but with nothing early-career | 66 |
| Boards returning nothing at all | 1 |

## ⚠ Adapters that failed

These are our bugs, not empty boards. Each one is lost coverage.

| Company | ATS | Token |
|:--|:--|:--|
| ClickHouse | greenhouse | `clickhouse` |

## Boards we read that had nothing early-career

These companies are hiring — just not for roles that clear the bar today.
A sample of what we saw and why we dropped it, so you can check our work
rather than take it on faith.

<details><summary><b>Cognition</b> — 94 postings read, none early-career</summary>

- `General Application - Cognition — title is not a technical role`
- `Research Engineer, Post-Training — research role without early-career marker`
- `Founding Recruiting Operations Coordinator - APJ — not a US location`
- `Founding GTM Recruiter - Japan — not a US location`
- `Event Marketer — title is not a technical role`
- `Deployed Engineer — not an early-career posting`

</details>

<details><summary><b>ElevenLabs</b> — 248 postings read, none early-career</summary>

- `Account Manager - India — not a US location`
- `Enterprise Solutions Engineer - North America — not an early-career posting`
- `Account Executive - Japan — not a US location`
- `Sales Development - United States — non-technical role`
- `Full-Stack Engineer (Back-End Leaning) — not an early-career posting`
- `Full-Stack Engineer (Front-End Leaning) — not an early-career posting`

</details>

<details><summary><b>Harvey</b> — 330 postings read, none early-career</summary>

- `Customer Success Manager, Mid Market, EMEA — not a US location`
- `Enterprise Customer Success Manager — non-technical role`
- `Senior Software Engineer, Core Infrastructure — senior/experienced role`
- `Senior Software Engineer, Frontend — senior/experienced role`
- `Applied Legal Researcher — not an early-career posting`
- `Account Executive, Enterprise, EMEA — not a US location`

</details>

<details><summary><b>Mistral AI</b> — 191 postings read, none early-career</summary>

- `AI Deployment Strategist - USA — not an early-career posting`
- `AI Scientist - Agentic Engineering — not a US location`
- `Field Marketing Manager - ASEAN — not a US location`
- `Software Engineer, New Grad — not a US location`
- `Recruiting Operations Manager — not a US location`
- `Head of Financial Systems — not a US location`

</details>

<details><summary><b>OpenAI</b> — 784 postings read, none early-career</summary>

- `Technical Program Manager, Compute Infrastructure — non-technical role`
- `Research Engineer — research role without early-career marker`
- `Account Director - Tokyo — not a US location`
- `Software Engineer, RL Training Infra — not an early-career posting`
- `Research Engineer, Retrieval & Search, Applied Engineering — research role without early-career marker`
- `Account Director, Startups — not a US location`

</details>

<details><summary><b>Perplexity</b> — 112 postings read, none early-career</summary>

- `Product Marketing Manager, Partnerships — non-technical role`
- `Member of Technical Staff (Software Engineer, Monetization) — senior/experienced role`
- `Associate Product Marketing Manager — non-technical role`
- `Member of Technical Staff (AI Inference Engineer) — senior/experienced role`
- `Member of Technical Staff (AI Infrastructure Engineer) — senior/experienced role`
- `Member of Technical Staff (Backend/Infrastructure Engineer, Search) — not a US location`

</details>

<details><summary><b>Poolside</b> — 15 postings read, none early-career</summary>

- `Member of Engineering (Pre-training / Data Research) — not a US location`
- `Member of Engineering (Post-training) — not a US location`
- `Member of Engineering (Evaluations) — not a US location`
- `Member of Engineering (Infrastructure) — not a US location`
- `Member of Engineering (Multimodality - Research Lead) — not a US location`
- `Member of Engineering (Evaluations / Engineering) — not a US location`

</details>

<details><summary><b>Reflection AI</b> — 49 postings read, none early-career</summary>

- `Member of Technical Staff - Pre-Training — senior/experienced role`
- `Member of Technical Staff - Post-Training — senior/experienced role`
- `Don't See Your Role? Apply Here! — title is not a technical role`
- `Member of Technical Staff - Safety — senior/experienced role`
- `Forward Deployed Engineer, Lead - LLM Post-training — senior/experienced role`
- `Strategy & Ops Generalist — title is not a technical role`

</details>

<details><summary><b>Thinking Machines Lab</b> — 38 postings read, none early-career</summary>

- `Research, General Agents — not an early-career posting`
- `Site Reliability Engineer, Post Training — not an early-career posting`
- `Network Engineer, Supercomputing — not an early-career posting`
- `Reliability Engineer, Supercomputing — not an early-career posting`
- `Safety Operations Lead — title is not a technical role`
- `Software Engineer, Systems Generalist — not an early-career posting`

</details>

<details><summary><b>Adobe</b> — 40 postings read, none early-career</summary>

- `Senior Manager, Digital Monetization Growth — title is not a technical role`
- `Enterprise Architect (Martech & AI) — not a US location`
- `AI Solutions Architect, Business Process Optimization — senior/experienced role`
- `Group Manager, Global Customer Operations & Growth — not a US location`
- `Sr. Manager, Cyber Threat Research & Intelligence — not a US location`
- `Senior Product Manager, Privacy Engineering — not a US location`

</details>

<details><summary><b>Airbnb</b> — 165 postings read, none early-career</summary>

- `Account Manager — not a US location`
- `Acquisition Manager — not a US location`
- `Acquisition Manager — not a US location`
- `Automation Engineer, Quality Engineering — not a US location`
- `Business Operations and Growth Lead — title is not a technical role`
- `Business Operations and Growth Lead — not a US location`

</details>

<details><summary><b>Brex</b> — 266 postings read, none early-career</summary>

- `Account Executive, Small Business — non-technical role`
- `Account Executive, Small Business — non-technical role`
- `Account Executive, Small Business — non-technical role`
- `Account Executive, YC — non-technical role`
- `Account Protection Specialist — not a US location`
- `AI Engineer, Product — not an early-career posting`

</details>

<details><summary><b>Bridgewater Associates</b> — 14 postings read, none early-career</summary>

- `2027 Investment Associate Intern — title is not a technical role`
- `Associate, Business Development Operations- Singapore — not a US location`
- `Associate, Client Service Operations — title is not a technical role`
- `Campus Talent Associate — title is not a technical role`
- `Client Investment Research Associate — quant role without early-career marker`
- `Data Associate — quant role without early-career marker`

</details>

<details><summary><b>Character.AI</b> — 13 postings read, none early-career</summary>

- `Machine Learning Infrastructure Engineer — not an early-career posting`
- `Research Engineer, Post-Training (All Industry Levels) — research role without early-career marker`
- `Software Engineer, Core Product — not an early-career posting`
- `Software Engineer, Backend — not an early-career posting`
- `Software Engineer, Monetization — not an early-career posting`
- `Software Engineer, Backend/Applied ML (Safety & Integrity) — not an early-career posting`

</details>

<details><summary><b>Chicago Trading Company (Lateral)</b> — 25 postings read, none early-career</summary>

- `AI Engineer — quant role without early-career marker`
- `Business Manager - Systematic — senior/experienced role`
- `Campus Relations & Events Associate — title is not a technical role`
- `C++ Engineering Lead – Delta Force — senior/experienced role`
- `Compliance Officer – Trade Surveillance — title is not a technical role`
- `C++ Senior Engineer - Market Data — senior/experienced role`

</details>

<details><summary><b>Decagon</b> — 139 postings read, none early-career</summary>

- `Enterprise Account Executive — non-technical role`
- `Director of Sales, Enterprise — non-technical role`
- `Senior Research Engineer — senior/experienced role`
- `Senior Agent Product Manager — title is not a technical role`
- `Engineering Manager, Agent Orchestration — senior/experienced role`
- `Strategic Account Director - West — not a US location`

</details>

<details><summary><b>Discord</b> — 44 postings read, none early-career</summary>

- `Advertising Operations Manager — title is not a technical role`
- `Commercial Policy Lead, Brand Safety & Malware — title is not a technical role`
- `Director of Engineering, Safety — senior/experienced role`
- `Engineering Manager, Machine Learning (Safety) — senior/experienced role`
- `Engineering Manager, Notifications — senior/experienced role`
- `Engineering Manager, Platform Security — senior/experienced role`

</details>

<details><summary><b>Linear</b> — 30 postings read, none early-career</summary>

- `Senior / Staff Fullstack Engineer — not a US location`
- `Senior / Staff Fullstack Engineer — not a US location`
- `Senior / Staff Product Engineer — not a US location`
- `Senior / Staff Product Engineer — not a US location`
- `Product Engineer — not a US location`
- `Product Manager — not a US location`

</details>

<details><summary><b>Lovable</b> — 82 postings read, none early-career</summary>

- `Data Platform Engineer — not a US location`
- `Research Engineering Lead — not a US location`
- `Product Designer — not a US location`
- `Brand Designer, Web — non-technical role`
- `Solutions Architect, San Francisco — title is not a technical role`
- `Staff / Principal Design Engineer, Web — senior/experienced role`

</details>

<details><summary><b>Luma AI</b> — 45 postings read, none early-career</summary>

- `Research Scientist / Engineer – Performance Optimization — research role without early-career marker`
- `Account Executive – Entertainment — non-technical role`
- `Research Scientist / Engineer – Reinforcement Learning Infrastructure — research role without early-career marker`
- `Research Scientist / Engineer – Training Infrastructure — research role without early-career marker`
- `Senior Site Reliability Engineer — senior/experienced role`
- `Creative Technologist — not a US location`

</details>

<details><summary><b>Mercury</b> — 59 postings read, none early-career</summary>

- `Account Executive — non-technical role`
- `Counsel, Product & Regulatory - Payments & AML — title is not a technical role`
- `Customer Support Specialist — non-technical role`
- `Customer Support Systems & Analytics Architect — senior/experienced role`
- `Executive Producer - Brand Creative — title is not a technical role`
- `Head of Data Engineering & Platform — senior/experienced role`

</details>

<details><summary><b>MongoDB</b> — 390 postings read, none early-career</summary>

- `Account Development Representative — not a US location`
- `Account Development Representative — not a US location`
- `Account Development Representative — not a US location`
- `Account Development Representative — not a US location`
- `Account Development Representative - English Speaking — not a US location`
- `Account Development Representative, French Speaking — not a US location`

</details>

<details><summary><b>NVIDIA</b> — 40 postings read, none early-career</summary>

- `Senior NPI Program Manager — non-technical role`
- `Director, Technical Program Management, Enterprise & AI — senior/experienced role`
- `Automotive Account Manager — not a US location`
- `Senior System Design Engineer — senior/experienced role`
- `Senior Supercomputing and Higher Education Account Manager — not a US location`
- `ASIC Design Engineer - New College Grad 2026 — not an early-career posting`

</details>

<details><summary><b>Plaid</b> — 102 postings read, none early-career</summary>

- `Strategic Initiatives — title is not a technical role`
- `Technical Support Engineer — not an early-career posting`
- `Senior Data Engineer - Data Engineering — senior/experienced role`
- `Strategic Finance Lead, Product & Engineering — senior/experienced role`
- `Staff Product Manager - AI Foundations — senior/experienced role`
- `Account Executive - Fintech Named — non-technical role`

</details>


## Every company

| Company | Tier | Board | Fetched | Published | Status |
|:--|:-:|:--|--:|--:|:--|
| [Scale AI](https://scale.com/careers) | 0 | `greenhouse:scaleai` | 221 | 4 | ✅ ok |
| [Stripe](https://stripe.com/jobs/university) | 0 | `greenhouse:stripe` | 607 | 4 | ✅ ok |
| [Anthropic](https://www.anthropic.com/careers) | 0 | `greenhouse:anthropic` | 594 | 3 | ✅ ok |
| [Cohere](https://cohere.com/careers) | 0 | `ashby:cohere` | 142 | 3 | ✅ ok |
| [Databricks](https://www.databricks.com/company/careers) | 0 | `greenhouse:databricks` | 858 | 3 | ✅ ok |
| [Figure AI](https://www.figure.ai/careers) | 0 | `greenhouse:figureai` | 103 | 3 | ✅ ok |
| [Sierra](https://sierra.ai/careers) | 0 | `ashby:sierra` | 209 | 3 | ✅ ok |
| [Mercor](https://mercor.com/careers) | 0 | `ashby:mercor` | 100 | 2 | ✅ ok |
| [Anysphere (Cursor)](https://cursor.com/careers) | 0 | `ashby:cursor` | 128 | 1 | ✅ ok |
| [Physical Intelligence](https://www.physicalintelligence.company/careers) | 0 | `ashby:physicalintelligence` | 34 | 1 | ✅ ok |
| [xAI](https://x.ai/careers) | 0 | `greenhouse:xai` | 253 | 1 | ✅ ok |
| [Citadel](https://www.citadel.com/careers/open-opportunities/?experience-filter=internships) | 0 | — | — | — | 🔗 link-only |
| [Citadel Securities](https://www.citadelsecurities.com/careers/open-opportunities/?experience-filter=internships) | 0 | — | — | — | 🔗 link-only |
| [Cognition](https://cognition.ai/careers) | 0 | `ashby:cognition` | 94 | — | ○ none matched |
| [D. E. Shaw](https://www.deshaw.com/careers) | 0 | — | — | — | 🔗 link-only |
| [ElevenLabs](https://elevenlabs.io/careers) | 0 | `ashby:elevenlabs` | 248 | — | ○ none matched |
| [Harvey](https://www.harvey.ai/careers) | 0 | `ashby:harvey` | 330 | — | ○ none matched |
| [Jane Street](https://www.janestreet.com/join-jane-street/open-roles/?type=student) | 0 | — | — | — | 🔗 link-only |
| [Mistral AI](https://mistral.ai/careers) | 0 | `ashby:mistral.ai` | 191 | — | ○ none matched |
| [OpenAI](https://openai.com/careers) | 0 | `ashby:openai` | 784 | — | ○ none matched |
| [Optiver](https://optiver.com/join-us/jobs/?location=us) | 0 | — | — | — | 🔗 link-only |
| [Perplexity](https://www.perplexity.ai/careers) | 0 | `ashby:perplexity` | 112 | — | ○ none matched |
| [Poolside](https://poolside.ai/careers) | 0 | `ashby:poolside` | 15 | — | ○ none matched |
| [Reflection AI](https://reflection.ai/careers) | 0 | `ashby:reflectionai` | 49 | — | ○ none matched |
| [Thinking Machines Lab](https://thinkingmachines.ai/) | 0 | `ashby:thinkingmachines` | 38 | — | ○ none matched |
| [Two Sigma](https://careers.twosigma.com/careers/OpenRoles) | 0 | — | — | — | 🔗 link-only |
| [Palantir](https://www.palantir.com/careers/) | 1 | `lever:palantir` | 310 | 48 | ✅ ok |
| [SpaceX](https://www.spacex.com/careers/) | 1 | `greenhouse:spacex` | 2358 | 30 | ✅ ok |
| [Jump Trading](https://www.jumptrading.com/careers/) | 1 | `greenhouse:jumptrading` | 106 | 20 | ✅ ok |
| [Akuna Capital](https://akunacapital.com/careers) | 1 | `greenhouse:akunacapital` | 37 | 19 | ✅ ok |
| [Applied Intuition](https://www.appliedintuition.com/careers) | 1 | `ashby:applied` | 282 | 18 | ✅ ok |
| [IMC Trading](https://careers.imc.com/us/en) | 1 | `greenhouse:imc` | 169 | 11 | ✅ ok |
| [Point72 / Cubist](https://careers.point72.com/) | 1 | `greenhouse:point72` | 219 | 11 | ✅ ok |
| [Anduril Industries](https://www.anduril.com/careers/) | 1 | `greenhouse:andurilindustries` | 2176 | 10 | ✅ ok |
| [DRW](https://drw.com/work-at-drw/) | 1 | `greenhouse:drweng` | 157 | 10 | ✅ ok |
| [Old Mission Capital](https://www.oldmissioncapital.com/careers/) | 1 | `greenhouse:oldmissioncapital` | 38 | 9 | ✅ ok |
| [Coinbase](https://www.coinbase.com/careers) | 1 | `greenhouse:coinbase` | 212 | 8 | ✅ ok |
| [AQR Capital](https://www.aqr.com/Careers) | 1 | `greenhouse:aqr` | 54 | 7 | ✅ ok |
| [Five Rings](https://fiverings.com/careers/) | 1 | `greenhouse:fiveringsllc` | 16 | 7 | ✅ ok |
| [Schonfeld](https://www.schonfeld.com/careers/) | 1 | `greenhouse:schonfeld` | 67 | 7 | ✅ ok |
| [Amazon](https://www.amazon.jobs/en/teams/internships-for-students) | 1 | `amazon:amazon` | 6 | 6 | ✅ ok |
| [Notion](https://www.notion.so/careers) | 1 | `ashby:notion` | 127 | 6 | ✅ ok |
| [Waymo](https://waymo.com/careers/) | 1 | `greenhouse:waymo` | 333 | 6 | ✅ ok |
| [Chicago Trading Company](https://www.chicagotrading.com/careers/) | 1 | `greenhouse:chicagotradingcampus` | 8 | 4 | ✅ ok |
| [PDT Partners](https://www.pdtpartners.com/careers) | 1 | `greenhouse:pdtpartners` | 9 | 4 | ✅ ok |
| [Roblox](https://careers.roblox.com/) | 1 | `greenhouse:roblox` | 228 | 4 | ✅ ok |
| [Tower Research Capital](https://www.tower-research.com/open-positions/) | 1 | `greenhouse:towerresearchcapital` | 84 | 4 | ✅ ok |
| [Etched](https://www.etched.com/careers) | 1 | `ashby:etched` | 105 | 3 | ✅ ok |
| [Fireworks AI](https://fireworks.ai/careers) | 1 | `ashby:fireworks` | 72 | 3 | ✅ ok |
| [Netflix](https://explore.jobs.netflix.net/careers) | 1 | `eightfold:netflix.com` | 485 | 3 | ✅ ok |
| [Datadog](https://careers.datadoghq.com/) | 1 | `greenhouse:datadog` | 428 | 2 | ✅ ok |
| [Headlands Technologies](https://www.headlandstech.com/careers/) | 1 | `greenhouse:headlandstechnologiesllc` | 7 | 2 | ✅ ok |
| [LangChain](https://www.langchain.com/careers) | 1 | `ashby:langchain` | 108 | 2 | ✅ ok |
| [Vercel](https://vercel.com/careers) | 1 | `greenhouse:vercel` | 84 | 2 | ✅ ok |
| [Baseten](https://www.baseten.co/careers) | 1 | `ashby:baseten` | 90 | 1 | ✅ ok |
| [Cerebras](https://www.cerebras.ai/careers) | 1 | `ashby:cerebras` | 112 | 1 | ✅ ok |
| [Cloudflare](https://www.cloudflare.com/careers/) | 1 | `greenhouse:cloudflare` | 342 | 1 | ✅ ok |
| [DoorDash](https://careers.doordash.com/) | 1 | `greenhouse:doordashusa` | 452 | 1 | ✅ ok |
| [Figma](https://www.figma.com/careers/) | 1 | `greenhouse:figma` | 158 | 1 | ✅ ok |
| [Hudson River Trading](https://www.hudsonrivertrading.com/careers/) | 1 | `greenhouse:hrttalentcommunity` | 3 | 1 | ✅ ok |
| [Modal](https://modal.com/careers) | 1 | `ashby:modal` | 31 | 1 | ✅ ok |
| [Radix Trading](https://www.radix-trading.com/careers) | 1 | `greenhouse:radixuniversity` | 8 | 1 | ✅ ok |
| [Replit](https://replit.com/careers) | 1 | `ashby:replit` | 78 | 1 | ✅ ok |
| [Squarepoint Capital](https://www.squarepoint-capital.com/careers) | 1 | `greenhouse:squarepointcapital` | 92 | 1 | ✅ ok |
| [The Voleon Group](https://voleon.com/careers/) | 1 | `ashby:voleon` | 57 | 1 | ✅ ok |
| [XTX Markets](https://www.xtxmarkets.com/careers/) | 1 | `greenhouse:xtxmarketstechnologies` | 11 | 1 | ✅ ok |
| [AMD](https://careers.amd.com/) | 1 | — | — | — | 🔗 link-only |
| [Adobe](https://careers.adobe.com/) | 1 | `workday:adobe` | 40 | — | ○ none matched |
| [Airbnb](https://careers.airbnb.com/) | 1 | `greenhouse:airbnb` | 165 | — | ○ none matched |
| [Apple](https://jobs.apple.com/en-us/search?team=internships-STDNT-INTRN) | 1 | — | — | — | 🔗 link-only |
| [Brex](https://www.brex.com/careers) | 1 | `greenhouse:brex` | 266 | — | ○ none matched |
| [Bridgewater Associates](https://www.bridgewater.com/working-at-bridgewater) | 1 | `greenhouse:bridgewater89` | 14 | — | ○ none matched |
| [Character.AI](https://character.ai/careers) | 1 | `ashby:character` | 13 | — | ○ none matched |
| [Chicago Trading Company (Lateral)](https://www.chicagotrading.com/careers/) | 1 | `greenhouse:chicagotrading` | 25 | — | ○ none matched |
| [ClickHouse](https://clickhouse.com/company/careers) | 1 | `greenhouse:clickhouse` | — | — | ⚠️ FETCH FAILED |
| [Decagon](https://decagon.ai/careers) | 1 | `ashby:decagon` | 139 | — | ○ none matched |
| [Discord](https://discord.com/careers) | 1 | `greenhouse:discord` | 44 | — | ○ none matched |
| [Google](https://www.google.com/about/careers/applications/jobs/results/?target_level=INTERN_AND_APPRENTICE&target_level=EARLY) | 1 | — | — | — | 🔗 link-only |
| [Linear](https://linear.app/careers) | 1 | `ashby:linear` | 30 | — | ○ none matched |
| [Lovable](https://lovable.dev/careers) | 1 | `ashby:lovable` | 82 | — | ○ none matched |
| [Luma AI](https://lumalabs.ai/careers) | 1 | `ashby:lumaai` | 45 | — | ○ none matched |
| [Mercury](https://mercury.com/jobs) | 1 | `greenhouse:mercury` | 59 | — | ○ none matched |
| [Meta](https://www.metacareers.com/jobs?roles[0]=Internship&roles[1]=University%20Grad) | 1 | — | — | — | 🔗 link-only |
| [Microsoft](https://jobs.careers.microsoft.com/global/en/search?exp=Students%20and%20graduates) | 1 | — | — | — | 🔗 link-only |
| [Millennium](https://career.mlp.com/careers) | 1 | — | — | — | 🔗 link-only |
| [MongoDB](https://www.mongodb.com/company/careers) | 1 | `greenhouse:mongodb` | 390 | — | ○ none matched |
| [NVIDIA](https://www.nvidia.com/en-us/about-nvidia/careers/) | 1 | `workday:nvidia` | 40 | — | ○ none matched |
| [Plaid](https://plaid.com/careers/) | 1 | `ashby:plaid` | 102 | — | ○ none matched |
| [Qube Research & Technologies](https://www.qube-rt.com/careers) | 1 | `greenhouse:quberesearchandtechnologies` | 196 | — | ○ none matched |
| [Radix Trading (Experienced)](https://www.radix-trading.com/careers) | 1 | `greenhouse:radixexperienced` | 7 | — | ○ none matched |
| [Ramp](https://ramp.com/careers) | 1 | `ashby:ramp` | 146 | — | ○ none matched |
| [Reddit](https://redditinc.com/careers) | 1 | `greenhouse:reddit` | 146 | — | ○ none matched |
| [Robinhood](https://careers.robinhood.com/) | 1 | `greenhouse:robinhood` | 130 | — | ○ none matched |
| [Runway](https://runwayml.com/careers) | 1 | `ashby:runway` | 4 | — | ○ none matched |
| [SIG (Susquehanna)](https://careers.sig.com/us-campus/jobs) | 1 | — | — | — | 🔗 link-only |
| [Salesforce](https://careers.salesforce.com/) | 1 | `workday:salesforce` | 38 | — | ○ none matched |
| [SambaNova](https://sambanova.ai/careers) | 1 | `greenhouse:sambanovasystems` | 57 | — | ○ none matched |
| [Snowflake](https://careers.snowflake.com/us/en/search-results) | 1 | — | — | — | 🔗 link-only |
| [Suno](https://suno.com/careers) | 1 | `ashby:suno` | 66 | — | ○ none matched |
| [Supabase](https://supabase.com/careers) | 1 | `ashby:supabase` | 59 | — | ○ none matched |
| [Tenstorrent](https://tenstorrent.com/careers) | 1 | `greenhouse:tenstorrent` | 129 | — | ○ none matched |
| [Together AI](https://www.together.ai/careers) | 1 | `greenhouse:togetherai` | 59 | — | ○ none matched |
| [Vanta](https://www.vanta.com/careers) | 1 | `ashby:vanta` | 104 | — | ○ none matched |
| [Verkada](https://www.verkada.com/careers/) | 2 | `greenhouse:verkada` | 294 | 12 | ✅ ok |
| [Belvedere Trading](https://belvederetrading.com/careers/) | 2 | `lever:belvederetrading` | 14 | 5 | ✅ ok |
| [Samsara](https://www.samsara.com/company/careers/) | 2 | `greenhouse:samsara` | 245 | 3 | ✅ ok |
| [Intel](https://jobs.intel.com/) | 2 | `workday:intel` | 20 | 2 | ✅ ok |
| [Nuro](https://www.nuro.ai/careers) | 2 | `greenhouse:nuro` | 110 | 2 | ✅ ok |
| [Dropbox](https://jobs.dropbox.com/) | 2 | `greenhouse:dropbox` | 43 | 1 | ✅ ok |
| [Elastic](https://www.elastic.co/careers) | 2 | `greenhouse:elastic` | 344 | 1 | ✅ ok |
| [Flexport](https://www.flexport.com/careers/) | 2 | `greenhouse:flexport` | 172 | 1 | ✅ ok |
| [Grafana Labs](https://grafana.com/about/careers/) | 2 | `greenhouse:grafanalabs` | 125 | 1 | ✅ ok |
| [Postman](https://www.postman.com/company/careers/) | 2 | `greenhouse:postman` | 63 | 1 | ✅ ok |
| [Affirm](https://www.affirm.com/careers) | 2 | `greenhouse:affirm` | 161 | — | ○ none matched |
| [Airtable](https://www.airtable.com/careers) | 2 | `greenhouse:airtable` | 16 | — | ○ none matched |
| [Amplitude](https://amplitude.com/careers) | 2 | `greenhouse:amplitude` | 37 | — | ○ none matched |
| [Asana](https://asana.com/jobs) | 2 | `greenhouse:asana` | 103 | — | ○ none matched |
| [Attentive](https://www.attentive.com/careers) | 2 | `greenhouse:attentive` | 35 | — | ○ none matched |
| [Broadcom](https://www.broadcom.com/company/careers) | 2 | `workday:broadcom` | 288 | — | ○ none matched |
| [Chime](https://www.chime.com/careers/) | 2 | `greenhouse:chime` | 67 | — | ○ none matched |
| [ExodusPoint](https://www.exoduspoint.com/careers) | 2 | `greenhouse:exoduspoint` | 2 | — | ○ none matched |
| [Faire](https://www.faire.com/careers) | 2 | `greenhouse:faire` | 60 | — | ○ none matched |
| [Fastly](https://www.fastly.com/about/careers) | 2 | `greenhouse:fastly` | 46 | — | ○ none matched |
| [Gong](https://www.gong.io/careers/) | 2 | `greenhouse:gongio` | 88 | — | ○ none matched |
| [Gusto](https://gusto.com/about/careers) | 2 | `greenhouse:gusto` | 94 | — | ○ none matched |
| [Instacart](https://instacart.careers/) | 2 | `greenhouse:instacart` | 105 | — | ○ none matched |
| [Klaviyo](https://careers.klaviyo.com/) | 2 | `greenhouse:klaviyo` | 129 | — | ○ none matched |
| [Lyft](https://www.lyft.com/careers) | 2 | `greenhouse:lyft` | 160 | — | ○ none matched |
| [Man Group](https://www.man.com/careers) | 2 | `greenhouse:mangroup` | 50 | — | ○ none matched |
| [Marshall Wace](https://www.mwam.com/careers/) | 2 | `greenhouse:marshallwace` | — | — | ○ board empty |
| [Mixpanel](https://mixpanel.com/careers/) | 2 | `greenhouse:mixpanel` | 83 | — | ○ none matched |
| [Okta](https://www.okta.com/company/careers/) | 2 | `greenhouse:okta` | 300 | — | ○ none matched |
| [Pinterest](https://www.pinterestcareers.com/) | 2 | `greenhouse:pinterest` | 177 | — | ○ none matched |
| [Quadrature Capital](https://quadrature.ai/careers/) | 2 | `greenhouse:quadraturecapital` | 4 | — | ○ none matched |
| [Qualcomm](https://careers.qualcomm.com/) | 2 | — | — | — | 🔗 link-only |
| [Railway](https://railway.com/careers) | 2 | `ashby:railway` | 8 | — | ○ none matched |
| [Render](https://render.com/careers) | 2 | `ashby:render` | 34 | — | ○ none matched |
| [Sentry](https://sentry.io/careers/) | 2 | `ashby:sentry` | 42 | — | ○ none matched |
| [Tailscale](https://tailscale.com/careers) | 2 | `greenhouse:tailscale` | 29 | — | ○ none matched |
| [Temporal](https://temporal.io/careers) | 2 | `ashby:temporal` | 70 | — | ○ none matched |
| [Tesla](https://www.tesla.com/careers/search/?type=3) | 2 | — | — | — | 🔗 link-only |
| [Twilio](https://www.twilio.com/en-us/company/jobs) | 2 | `greenhouse:twilio` | 140 | — | ○ none matched |
| [Vatic Labs](https://vaticlabs.com/careers) | 2 | `greenhouse:vaticlabs` | 9 | — | ○ none matched |
| [Warp](https://www.warp.dev/careers) | 2 | `ashby:warp` | 19 | — | ○ none matched |
| [Zed](https://zed.dev/jobs) | 2 | `ashby:zed` | 1 | — | ○ none matched |

---

[Back to the boards](https://github.com/ricsign/Ricsign-New-Grads-Jobs-2027#boards)
